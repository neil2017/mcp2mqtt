...
本项目基于 [mcp2everything/mcp2mqtt](https://github.com/mcp2everything/mcp2mqtt) 进行二次开发，原项目版权归原作者所有。

### 原作者
- **项目地址**: [https://github.com/mcp2everything/mcp2mqtt](https://github.com/mcp2everything/mcp2mqtt)
- **许可证**: 请参考原项目 LICENSE 文件

### 本分支修改与优化

本分支在原项目基础上进行了以下修改和优化：

#### 🐛 Bug 修复
1. **MQTT Broker 地址配置错误** — 原配置中 `eclipse-mosquitto` 地址无法连接（端口未开放），已修正为可用的公共 Broker 地址
2. **YAML 缩进错误** — `commands` 段中 `tuya_control` 前存在多余空格导致解析异常，已修正
3. **订阅/发送顺序导致状态错乱** — `connect_and_send()` 中先订阅再发送，导致收到 retained 旧状态而非真实响应。已改为**先发送命令再订阅响应主题**
4. **硬编码工具分支** — 消除了 `set_pwm`/`get_pico_info`/`led_control` 的硬编码逻辑，所有工具统一走 `response_format` 模板

#### ✨ 功能增强
1. **MQTT Broker 诊断** — 新增 `diagnose_mqtt_broker()` 函数，连接失败时自动诊断 DNS 解析和 TCP 端口状态
2. **超时可配置** — 新增 `mqtt_timeout` 配置项（默认 10 秒），可在 config.yaml 中自定义
3. **状态查询功能** — `tuya_control` 工具新增 `state=query` 模式，可查询楼梯灯当前状态（读取 retained 消息）
4. **Docker 部署优化** — 修改 `docker-compose.yml` 支持卷挂载方式部署，无需重新构建镜像即可更新代码

#### 🧪 测试完善
1. 新增 `test_format_tool_message_for_generic_tool` 测试用例
2. 新增 `test_format_tool_message_for_generic_tool_missing_template` 测试用例
3. 新增 `test_diagnose_mqtt_broker_reachable/unreachable/dns_fail` 诊断测试
4. 新增 `test_tuya_control_query_mode` 查询模式测试
5. 更新 `responder.py` 模拟器支持 `tuya_control` 响应
